...
def embed_dict(file):
    ...
    return temp
# embeddings=embed_dict('input/word_embedding.txt')
# print(embeddings['W100000'])

# 读取train和test
def read_data(data):
    data=pd.merge(data,question[['qid','wid']],left_on='qid1',right_on='qid',how='left')
    data=pd.merge(data,question[['qid','wid']],left_on='qid2',right_on='qid',how='left')
    data.drop(['qid_x','qid_y'],axis=1,inplace=True)
    data.columns=['qid1', 'qid2', 'label', 'word1', 'word2']
    return data

# texts_to_sequences
def text2seq(q1,q2,tokenizer,MSL=25):
    return pad_sequences(tokenizer.texts_to_sequences(q1),maxlen=MSL),\
           pad_sequences(tokenizer.texts_to_sequences(q2),maxlen=MSL)
